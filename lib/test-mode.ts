...
export async function checkTestMode(): Promise<TestModeInfo> {
  ...
  try {
    ...
    // test 또는 test-locked 상태 모두 테스트 모드로 인식 (완료 안내 표시용)
    const isTestMode = user.customerStatus === 'test' || user.customerStatus === 'test-locked';
...
/**
 * 경로 보호: 사용자의 테스트 모드 상태와 경로가 일치하는지 확인
