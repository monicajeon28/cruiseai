...
export async function GET(request: NextRequest) {
  try {
    if (!YOUTUBE_API_KEY) {
      logger.warn('[YouTube API] YOUTUBE_API_KEY 환경 변수가 설정되지 않았습니다.');
    ...
    try {
      ...
      if (!channelResponse.ok) {
        ...
        logger.error('YouTube API Error (channels):', error);
        return NextResponse.json(
      ...
      if (!videosResponse.ok) {
        ...
        logger.error('YouTube API Error (search):', error);
        // 실패 시 기존 방식으로 폴백
        ...
        try {
          ...
          clearTimeout(playlistTimeoutId);
          if (playlistError.name === 'AbortError') {
            logger.error('[YouTube API] 타임아웃 발생 (playlist)');
          } else {
            logger.error('[YouTube API] Playlist fetch error:', playlistError);
          }
          return NextResponse.json({
            ok: true,
            channel: null,
      ...
      if (fetchError.name === 'AbortError') {
        logger.error('[YouTube API] 타임아웃 발생');
      } else {
        logger.error('[YouTube API] Fetch error:', fetchError);
      }
      // 타임아웃이나 에러 발생 시 빈 배열 반환하여 로딩이 멈추도록 함
      return NextResponse.json({
        ok: true,
        channel: null,
        videos: [],
    ...
    logger.error('[YouTube API] Error fetching YouTube videos:', error);
    // 에러 발생 시 빈 배열 반환하여 로딩이 멈추도록 함
    return NextResponse.json({
      ok: true,
      channel: null,
      videos: [],
