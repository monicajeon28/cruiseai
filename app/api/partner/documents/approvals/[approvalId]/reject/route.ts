...
export async function POST(
    ...
    try {
        ...
        if (!profile || profile.type !== 'BRANCH_MANAGER') {
            return NextResponse.json({ ok: false, error: 'Only Branch Managers can reject requests' }, { status: 403 });
        }
        ...
        if (!teamRelation && requesterProfile.id !== profile.id) {
            return NextResponse.json({ ok: false, error: 'You can only reject requests from your team' }, { status: 403 });
        }
        ...
        console.error('[Partner Document Reject] Error:', error);
