    ...
    if (mode === 'go') {
      ...
      if (destination) {
        ...
        if (dests.length > 0) {
          ...
          // Transit option
          links += `- [대중교통 길찾기](${gmapsDir(originQ, dest.name_ko || dest.name, 'transit')})\n`;
          // Driving option
          links += `- [자동차 길찾기](${gmapsDir(originQ, dest.name_ko || dest.name, 'driving')})\n`;
      ...
      return [{ role: 'assistant', text: responseText }] as Msg[];
    }

    // 기존 로직 유지 (show, free 모드)
    return [{ role: 'assistant', text: `You said: ${payload.text}` }] as Msg[];
  }, []);
